    ...
    [].forEach.call(navbar, function(el, i) {
      addListenerMulti(el, "mouseenter mouseleave click", function(e) {
        if (window.innerWidth > 991.98) {
          ...
          e.type === "mouseenter"
            ? (instance.show(), dropdown.setAttribute("aria-expanded", true))
            : (instance.hide(),
              dropdown.blur(),
  ...
  function init(elem, index) {
    ...
    var parents = closest(elem, "section"),
      ...
      dataOptions = JSON.parse(
        elem
          .querySelector(".swiper-container:not(.swiper-thumbs)")
    ...
    if (hasThumb) {
      var dataOptionsThumbs = JSON.parse(
        elem.querySelector(".swiper-thumbs").getAttribute("data-options")
      );
      ...
      var swiperThumbs = new Swiper(
        elem.querySelector(".swiper-thumbs"),
        mergedOptions
      );
    }
    ...
    var swiper = new Swiper(
      elem.querySelector(".swiper-container:not(.swiper-thumbs)"),
      mergedOptions
    );
  }
...
var TypedJS = (function() {
  ...
  // Events
  if (typed.length > 0) {
    fetchInject(["../assets/vendor/typed.js/lib/typed.min.js"]).then(() => {
...
var Photoswipe = (function() {
  ...
  var openPhotoSwipe = function(
    ...
    var dataOptions =
      JSON.parse(galleryElement.parentNode.getAttribute("data-options")) ||
      JSON.parse(galleryElement.getAttribute("data-options"));
    ...
    var options = {
      ...
      preload: [1, 3],
      galleryUID:
        galleryElement.parentNode.getAttribute("data-pswp-uid") ||
        galleryElement.getAttribute("data-pswp-uid"),
      getThumbBoundsFn: function(index) {
        var thumbnail =
            items[index].el.getElementsByTagName("img")[0] ||
            items[index].el.getElementsByTagName("svg")[0],
          pageYScroll =
    ...
    gallery.listen("afterChange", function() {
      if (gallery.options.getNumItemsFn() > 1) {
        ...
        ssButtonClass.addEventListener(
          "click",
          function(event) {
...
var StickyElement = (function() {
  ...
  if (isExist(stickyItem)) {
    fetchInject(["../assets/vendor/sticky-js/dist/sticky.min.js"]).then(() => {
      init();
    });
  }
})();
...
var CounterUp = (function() {
  ...
  // Events
  if (countup.length > 0) {
    fetchInject(["../assets/vendor/countup.js/dist/countUp.min.js"]).then(
      () => {
...
var FloatingObjects = (function() {
  ...
  var transitionDurationProperty = getSupportedPropertyName(
    transitionDurations
  );
  var transforms = [
    "transform",
  ...
  function setInitialProperties() {
    for (var i = 0; i < objects.length; i++) {
      ...
      object.style.opacity = 0.005 + Math.random() * 0.06;
  ...
  function manageAnimations(elem) {
    setTimeout(function() {
      ...
      if (isExist(navbarCollapse)) {
        ...
        navbarCollapse.style.maxHeight =
          "calc(100% - 6.25rem - " + alertHeight + "px)";
      }
    }, 250);
  }
...
var Mapbox = (function() {
  ...
  function init(elem) {
    ...
    mapboxgl.accessToken =
      "pk.eyJ1IjoiYWxlc2FuY2hlenIiLCJhIjoiY2tsc2JyajFlMDB6ODJ3bnFoYnMweGdsMyJ9.YmH6BgHbtGrbu3J0GLXRLA"; // Create a new access token on https://account.mapbox.com/access-tokens/
    ...
    elem.addEventListener(
      ...
      function(e) {
        ...
        var linkGroupName = elem.getAttribute("data-link-group"),
          linkGroups = document.querySelectorAll(
            '[data-link-group="' + linkGroupName + '"]'
          );
        ...
        if (
          ...
          elem.getAttribute("href").slice(0, 1) == "#"
        ) {
          e.preventDefault();
        }
      },
    ...
    window.addEventListener(
      ...
      function(e) {
        ...
        if (pageYScroll > 10) {
          ...
          logo.querySelectorAll("path")[0].style.fill = getCssVariable(
            "--primary"
          );
          logo.querySelectorAll("path")[1].style.stroke = getCssVariable(
            "--primary"
          );
        } else {
